...
# Rideshare service model, but with vetting that decreases the number of malicious drivers.  

# Author: Ian Roberts
# Date of last Update: 2022-06-28

# Drivers have a "vetting efficacy" parameter, which determines what proportion of malicious drivers are caught
# by the vetting process. It decreases the number of malicious drivers proportionally. 
...
class Rider:
    ...
    vettingEfficacy = 0.5               #PROBABILITY THAT A MALICIOUS DRIVER IS CAUGHT BY VETTING
    ...
    def __init__(self, board):
        ...
        if (r.random() < self.probMale):
            self.male = True
            if (r.random() < (board.probMaliciousGivenMan * (1 - self.vettingEfficacy))):
                self.isMalicious = True
                if (r.random() < board.mTw):
                    self.targetWomen = True
                else:
            ...
            self.male = False
            if (r.random() < (board.probMaliciousGivenWoman * (1 - self.vettingEfficacy))):
                self.isMalicious = True
                if (r.random() < board.wTw):
                    self.targetWomen = True
                else:
